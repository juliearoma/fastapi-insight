    ...
    try:
        ...
        if user is None:
            ...
            user_table = Table(
                "user",
                metadata,
                Column("id", Integer, primary_key=True, autoincrement=True, nullable=False),  # Integer primary key
                ...
                Column("profile_image_url", String, default="https://profileimageurl.com", nullable=True),  # default 값과 nullable=True
                Column("uuid", UUID(as_uuid=True), default=uuid.uuid4, unique=True),  # UUID는 primary_key=False
                Column("created_at", DateTime(timezone=True), default=lambda: datetime.now(UTC), nullable=False),
                Column("updated_at", DateTime, nullable=True),  # nullable=True
                Column("deleted_at", DateTime, nullable=True),  # nullable=True
                Column("is_deleted", Boolean, default=False, index=True),
                Column("is_superuser", Boolean, default=False),
                Column("tier_id", Integer, ForeignKey("tier.id"), index=True, nullable=False)  # 명시적 nullable 설정
            )
